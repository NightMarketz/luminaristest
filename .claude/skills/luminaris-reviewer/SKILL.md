...
>
> **⛓️ [REV-008] Cabeçalho `atomicUntil` (Contrato §2.3, `AC-2.3-2`) — check cross-cutting, FAIL direto.** Se o diff cria ou altera um service que chama `.postEntry(` fora do `PostingService` — com ou sem subrazão (AP/AR/estoque/imobilizado/banco); sem, `commit 2 — nenhum` — sem que o **primeiro JSDoc do arquivo** contenha `atomicUntil:` e as 4 linhas `commit 1 — razão` · `commit 2 — subrazão` · `reconcile —` · `fora da tx —` → FAIL. FAIL também se (a) alguma linha cita teste que não existe ou que não assere o que a linha diz (classe `comentario-de-teste-afirma-o-que-nao-assere` — em reconcile, a asserção é sobre a **segunda** chamada); (b) o diff tenta razão + subrazão na "mesma tx" (`postEntry` abre a própria tx raiz) ou compensa com `try/catch` + delete; (c) introduz engine/fila/plugin para orquestrar isso (motor de domínio é decisão rejeitada — `docs/adr/ADR-DOMAIN-MOTOR-rejected.md`). Exclusões: `PostingService` (é o commit 1) e bridges DynamicTable no controller (`crmController.ts:92-107`). Evidência objetiva (Git Bash): `grep -L "atomicUntil:" $(grep -rlE "\.postEntry\(" server/src/features/*/services --include=*.ts | grep -v -e __tests__ -e PostingService)` restrito aos arquivos do diff deve ser **vazio**; após o PR-B, `cd server && npx jest atomicUntil.boundary`. Serviços pré-existentes sem cabeçalho são dívida registrada (GAP-MAP Nível 3, fila 9), não FAIL do diff que não os toca.
>
...
> **Service Prisma que chama `postEntry` (com ou sem subrazão):** primeiro JSDoc com o cabeçalho `atomicUntil` (5 linhas) + os 3 testes por linha, senão FAIL — ver `[REV-008]` acima (Contrato §2.3).
...
- [ ] Nova rota importada e registrada em `server/src/routes/index.ts`
  ...
  ```
- [ ] **`middleware/auth.ts` NÃO editado para proteger a rota** — auth é deny-by-default (`/api/*` já exige JWT; o array `protectedApiPaths` não existe mais). Diff que re-adiciona allowlist de proteção por rota = **FAIL** (regressão de modelo). Se a feature tem rota **pública**, a exceção deve estar como regra `{ path, method, match }` em `publicApiRoutes` — e SÓ ela.
  ```bash
  grep -n "protectedApiPaths" server/src/middleware/auth.ts   # deve ser VAZIO (deny-by-default)
  ```
...
- **[REV-008] Service com `postEntry` + subrazão sem cabeçalho `atomicUntil` = FAIL** — Contrato §2.3; 2 commits + reconcile, nunca "mesma tx", nunca engine
