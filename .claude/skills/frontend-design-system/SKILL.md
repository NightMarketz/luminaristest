...
description: Aplica a linguagem visual real do Luminaris (Galaxy theme — tokens `lumi-*`/`neutral-*`, superfícies dark charcoal, tipografia `font-black`/`font-semibold` e componentes-assinatura como GradientHeader/ScoreGauge/StatusBadge/BantBars) para que telas/componentes gerados fiquem on-brand, não Tailwind genérico. Use ao gerar ou reestilizar qualquer UI (card, header, badge, KPI tile, gauge), sempre que outra skill de frontend criar componente/tela, ou quando aparecerem termos como zinc, rounded, dark mode, neutral, on-brand, design system, estilizar, galaxy theme. Domínio/arquivos: my-app/features/<module>/components/**/*.tsx.
argument-hint: "[componente ou tela a estilizar]"
allowed-tools: Read, Grep, Glob, Write, Edit
user-invocable: false
compatibility: Claude Code; requer o monorepo Luminaris (my-app/ com Tailwind + tsc). Skill de conhecimento de apoio aplicada por outras skills de frontend ao gerar UI. Sem efeitos externos — apenas gera/edita arquivos .tsx no repositório.
metadata:
...
**[FEDS-003]** Superfícies dark usam `neutral-*` (charcoal sem undertone azul), **nunca** `zinc-*`. Página/card = `dark:bg-neutral-900`; tile interno = `dark:bg-white/[0.03]`; borda padrão = `dark:border-neutral-800`.
...
- **[FEDS-004]** **`font-semibold` é o peso de corpo padrão do app** (151 usos — o mais comum) e **`font-black` é reservado para ênfase** (títulos/heros com `tracking-tight`, valores grandes de KPI, labels uppercase de seção). Use `font-semibold` à vontade em texto/linhas de tabela/labels normais; **não** troque todo `font-semibold` por `font-black`. Labels de seção/KPI = `text-[10px] font-black uppercase tracking-widest text-gray-400`; valores de KPI = `text-2xl font-black`.
- **[FEDS-002]** **Cantos:** **cards** = `rounded-2xl`/`rounded-3xl`; tiles = `rounded-2xl`; pills = `rounded-full`; **inputs/botões/filtros = `rounded-xl`/`rounded-lg`** (o raio mais comum do app — 74 arquivos; NÃO é off-brand). Off-brand é só usar `rounded-xl` num *card* que deveria ser `2xl`.

## Componentes-assinatura (copie estes padrões)

**[FEDS-005]** Componentes-assinatura do Galaxy theme (espelhe o kit `my-app/features/crm/components/ui/`): hero usa **GradientHeader** (gradiente + blur blobs), score usa **ScoreGauge** (SVG circular), status usa **StatusBadge** (pill), progresso usa **BantBars**. Não reinvente esses primitivos genéricos quando o padrão on-brand já existe.

### Card / superfície
**[FEDS-001]** Superfície de card = `bg-white dark:bg-neutral-900` (nunca `zinc-*`).
```tsx
...
**[FEDS-006]** Badges não são sólidos: fundo `color/10` + borda `color/20` + texto `color-600`, pill `rounded-full` com `text-[10px] font-black uppercase tracking-widest`.
```tsx
...
- **[FEDS-003] Não use `zinc-*` para superfícies dark** — o app usa `neutral-*` (charcoal sem undertone azul). `zinc-*` é o sinal nº1 de "Tailwind genérico" e deve ser **zero** no código gerado. O grep de verificação é `grep -rn "zinc-" <pasta-do-modulo>` e **deve retornar nada**. (Borda dark padrão = `dark:border-neutral-800`.)
- **[FEDS-002]** Não use `rounded-xl` **em cards** — cards são `rounded-2xl/3xl`. (Mas `rounded-xl/lg` em inputs/botões/filtros é correto — não confunda.)
...
- **[FEDS-004]** Não troque `font-semibold` por `font-black` em todo lugar — `font-semibold` é o peso de corpo normal; `font-black` é só para ênfase (títulos/valores/labels uppercase)
- Não escreva labels em case normal — labels de KPI/seção são `text-[10px] uppercase tracking-widest`
- **[FEDS-006]** Não faça badges sólidos — use o padrão `color/10` (fundo) + `color/20` (borda) + `color-600` (texto)
