...
>
> Existe porque worktrees isolados nunca conflitam no código *da feature* — conflitam nos **arquivos de registro que o contrato §3 obriga todo feature a tocar** ("registro em 2 toques" — `routes/index.ts` + `docs.paths.ts`; auth deny-by-default tirou `middleware/auth.ts` da lista de choke points de registro). "Paralelo sem conflito" não é spawnar N implementers e torcer; é **separar corpo de registro** e serializar só o registro. Este contrato torna esse fatiamento **repetível** (qualquer execução chega ao mesmo lote paralelo + delta serial).
...
- **Fase dependente = barrier (fechar-antes-de-começar).** Uma fase só inicia quando a fase de que depende **fecha por inteiro** — nunca sobrepõe. No harness isto é o `await` entre fases; a regra é: **jamais requisitar um worktree antes do predecessor fechar.** A **Fase B só começa depois de TODA a Fase A + review fechar**, então as branches aprovadas já existem e estão visíveis ao integrador (validado empiricamente — seam 2, smoke `wf_f7a4600f`).
- **⚠️ O barrier ordena EXECUÇÃO, NÃO controla a base do worktree (seam 1 — corrigido pós-smoke `wf_f7a4600f`).** Medido: com `isolation: worktree` o harness escolhe a base de cada worktree de forma **inconsistente** — num run de 2 slices, um nasceu do HEAD com a Fase 0, o **outro nasceu de `main` SEM ela**. Portanto **NÃO** vale assumir "o worktree da Fase A herda o schema da Fase 0 porque a Fase 0 fechou antes". A regra dura: a **Fase 0 devolve o SHA do commit de schema**, e **cada slice da Fase A GARANTE esse commit no seu worktree** (`git merge-base --is-ancestor <sha> HEAD`; se não, `git merge`/`cherry-pick` + `prisma generate`) **antes de construir** — se não conseguir trazê-lo, **PARA** (não constrói contra schema stale). Consequência secundária: como a Fase 0 roda `prisma migrate dev` (muta o dev.db), uma migração numa base que os worktrees não herdam fica órfã — cada worktree precisa do commit de migração (o `prisma generate` acima cobre o *type-check*; um run que exercite o DB precisa aplicar a migração no seu banco).
