...
for (const f of eslintJson(['--no-inline-config'])) {
  ...
  if (n) real[rel(f.filePath)] = n;
}

const problems = [];

// 1. Violação silenciada por caminho que não deixa marcador (inline config, override).
  ...
  if (n > accounted) {
    problems.push(
      `\`${file}\`: ${n - accounted} violação(ões) de camada silenciada(s) SEM diretiva de supressão.\n` +
        `  Causa típica: \`/* eslint <regra>: "off" */\` (inline config) ou override no config — nenhum dos\n` +
        `  dois deixa marcador, e o lint fica verde. Troque por \`eslint-disable-next-line <regra> -- DEBT: <tag>\`\n` +
        `  e registre no bloco debt-ledger, ou conserte a violação.`
    );
  }
}
