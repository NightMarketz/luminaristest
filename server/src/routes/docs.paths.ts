 ...
 *       responses:
 *         '201':
 *           description: ECF (Lucro Real) export job created
 *           content:
 *             application/json:
 *               schema:
 *                 type: object
 ...
 *     AccountingBindingActivateDefaultRequest:
 *       type: object
 *       required: [unitId]
 *       additionalProperties: false
 *       properties:
 *         unitId:                     { type: string }
 *         sectorKey:                  { type: string, description: "default 'beautySalon'; setores com binding padrão: beautySalon, aestheticClinic" }
 *         installChartIfEmpty:        { type: boolean, description: 'plano de contas vazio ⇒ instala o plano canônico antes do compile' }
 *         openCurrentPeriodIfMissing: { type: boolean, description: 'período do mês corrente ausente/FUTURE ⇒ seed-year + open' }
 *
 * paths:
 *
 ...
 *         '403': { $ref: '#/components/responses/ForbiddenError' }
 *
 *   /api/accounting-binding/activate-default:
 *     post:
 *       summary: Activate the sector's DEFAULT binding (server-side fixture) — HTTP twin of the activation CLI
 *       description: >-
 *         LAC-B. Idempotente (Active existente ⇒ status 'already-active', nada compilado). Pré-condições
 ...
 *         '403': { $ref: '#/components/responses/ForbiddenError' }
 *
 *   /api/bank-settlements:
 *     get:
 *       summary: List bank settlement proposals (BE-INCR-BANK-SETTLEMENT, nó F7)
 *       description: >-
 *         Offset-paginated listing of PROPOSED settlements built by /scan from UNMATCHED statement
 *         lines. Each item carries the line (date, signed amountCents, description) and the title
 *         (open balance RECOMPUTED on read, never persisted). Only /confirm produces a financial
 *         effect (cédula 10/09 resposta 20).
 *       tags: [Accounting]
 *       security: [{ bearerAuth: [] }]
 *       parameters:
 *         - { in: query, name: unitId, required: true, schema: { type: string } }
 *         - { in: query, name: statementId, required: false, schema: { type: string } }
 *         - { in: query, name: status, required: false, schema: { type: string, enum: [PENDING, CONFIRMING, CONFIRMED, REJECTED, FAILED, STALE] } }
 *         - { in: query, name: page, required: false, schema: { type: integer, default: 1 } }
 ...
 *         '403': { $ref: '#/components/responses/ForbiddenError' }
 *
 *   /api/bank-settlements/scan:
 *     post:
 *       summary: Scan a statement's UNMATCHED lines into settlement proposals (F-F7-1 a)
 *       description: >-
 *         For each UNMATCHED line of the statement, deterministic candidacy (BRIEF item 3): sign
 *         picks the side (negative = Payable, positive = Receivable), due date within [line − 30d,
 *         line + 5d], |line| == open balance (exact), < open (partial, #307) or ≤ open + 20% cap
 *         (charge, F-F7-5 a). Exactly ONE candidate creates a PENDING item; 0 or >1 are counted as
 *         none/ambiguous and skipped. Idempotent: a live item on the line is not duplicated. PENDING
 *         items whose line/title changed since are marked STALE (item 10).
 *       tags: [Accounting]
 *       security: [{ bearerAuth: [] }]
 *       requestBody:
 *         required: true
 ...
 *       responses:
 *         '200': { description: 'created, skippedExisting, ambiguous, none, stale' }
 *         '400': { $ref: '#/components/responses/BadRequestError' }
 *         '401': { $ref: '#/components/responses/UnauthorizedError' }
 *         '403': { $ref: '#/components/responses/ForbiddenError' }
 *         '404': { $ref: '#/components/responses/NotFoundError' }
 *
 *   /api/bank-settlements/{id}/confirm:
 *     post:
 *       summary: Confirm a proposal — the ONLY financial effect (items 6–8, F-F7-3/4 a)
 *       description: >-
 *         CAS PENDING→CONFIRMING closes the race (two confirms = one effect). Authoritative
 *         pre-check in a read transaction BEFORE any effect: item PENDING, line UNMATCHED with no
 *         active match, statement active, title balance re-read ≥ proposedCents, the account the
 *         method resolves to must be the statement's bank account, and — when chargeCents > 0 —
 ...
 *         '404': { $ref: '#/components/responses/NotFoundError' }
 *
 *   /api/bank-settlements/{id}/reject:
 *     post:
 *       summary: Reject a proposal (item 5) — no accounting effect
 *       description: PENDING → REJECTED with the operator reason (masked in the audit trail). The line stays UNMATCHED and eligible for manualMatch.
 *       tags: [Accounting]
 *       security: [{ bearerAuth: [] }]
 *       parameters:
 *         - { in: path, name: id, required: true, schema: { type: string } }
 ...
 *                 reason: { type: string, maxLength: 500 }
 *       responses:
 *         '200': { description: 'BankSettlementItemView (status REJECTED)' }
 *         '400': { $ref: '#/components/responses/BadRequestError' }
 *         '401': { $ref: '#/components/responses/UnauthorizedError' }
 *         '403': { $ref: '#/components/responses/ForbiddenError' }
 *         '404': { $ref: '#/components/responses/NotFoundError' }
 *
 *   /api/bank-settlements/{id}/retry:
 *     post:
 *       summary: Retry a FAILED confirmation from the first step without a saved id (item 9)
 *       description: FAILED → CONFIRMING → re-runs (i)–(iv) skipping steps whose id is already saved (settlementId, chargeEntryId) — never a second payment. CONFIRMED/REJECTED/PENDING → 400.
 *       tags: [Accounting]
 *       security: [{ bearerAuth: [] }]
 *       parameters:
 *         - { in: path, name: id, required: true, schema: { type: string } }
 ...
 *     get:
 *       summary: Read the per-scope accounting settings (AccountingScopeSettings, 2026-09-15)
 *       tags: [Accounting]
 *       security: [{ bearerAuth: [] }]
 *       parameters:
 *         - { in: query, name: unitId, required: true, schema: { type: string } }
 ...
 *         - { in: query, name: pendencias, required: false, schema: { type: boolean }, description: 'item 30 — true filtra só documentos com pendências; exige saleId ou status junto' }
 *       responses:
 ...
 *         '403': { $ref: '#/components/responses/ForbiddenError' }
 *
 *   /api/nfe/dfe/documents/{id}/consultar:
 *     post:
 *       summary: Re-query the partner for one document's current result (item 27, manual re-check)
 *       description: >-
 *         No-op (devolve a view atual) se o documento já estiver num status terminal
 *         (AUTHORIZED/REJECTED/CANCELLED) — só SENT/PROCESSING são re-consultados.
 *       tags: [Accounting]
 *       security: [{ bearerAuth: [] }]
 *       parameters:
 *         - { in: path, name: id, required: true, schema: { type: string } }
 ...
 *       responses:
 *         '200': { description: 'FiscalDocumentView (atualizada, se houve transição)' }
 *         '400': { $ref: '#/components/responses/BadRequestError' }
 *         '401': { $ref: '#/components/responses/UnauthorizedError' }
 *         '403': { $ref: '#/components/responses/ForbiddenError' }
 *
 *   /api/nfe/dfe/documents/{id}/reenviar:
 *     post:
 *       summary: Resend a REJECTED document as a new attempt (item 26)
 *       description: >-
 *         Só de status REJECTED. Payload remontado (perfil/venda corrigidos), tentativa n+1 com ref
 *         novo; a tentativa n permanece intacta. A numeração da DPS NÃO é reconsumida.
 *       tags: [Accounting]
 *       security: [{ bearerAuth: [] }]
 *       parameters:
 *         - { in: path, name: id, required: true, schema: { type: string } }
 ...
 *             schema: { type: object, required: [unitId], properties: { unitId: { type: string } } }
 *       responses:
 *         '200': { description: 'FiscalDocumentView (status SENT, nova tentativa)' }
 *         '400': { $ref: '#/components/responses/BadRequestError' }
 *         '401': { $ref: '#/components/responses/UnauthorizedError' }
 *         '403': { $ref: '#/components/responses/ForbiddenError' }
 *
 *   /api/nfe/dfe/documents/{id}/cancelar:
 *     post:
 *       summary: Cancel an AUTHORIZED document (item 29, Anexo II e101101)
 *       description: >-
 *         Só de status AUTHORIZED. Nunca toca a venda nem o razão. OUT_OF_WINDOW/REJECTED do
 *         parceiro viram 409 tipado; CANCELLED retira a proveniência anexada (F-DFE-18, soft-delete).
 *       tags: [Accounting]
 *       security: [{ bearerAuth: [] }]
 *       parameters:
 *         - { in: path, name: id, required: true, schema: { type: string } }
 ...
 *             schema:
 *               type: object
 *               required: [unitId, cMotivo, xMotivo]
 *               properties:
 *                 unitId: { type: string }
 *                 cMotivo: { type: integer, enum: [1, 2, 9], description: '1 erro na emissão | 2 serviço não prestado | 9 outros' }
 ...
 *       description: >-
 *         Rota PÚBLICA (publicApiRoutes) — sem Bearer. Assinatura inválida, `:partner` diferente do
 *         parceiro habilitado, ou adaptador sem capabilities.webhook ⇒ 401 sem efeito (0 escrita).
 *         Null/File (os únicos adaptadores deste BRIEF) ⇒ 401 sempre — nenhum tem webhook real.
 *       tags: [Accounting]
 *       security: []
 *       parameters:
 *         - { in: path, name: partner, required: true, schema: { type: string } }
 *       responses:
 *         '200': { description: 'assinatura válida — re-consulta disparada (ou nada a fazer)' }
 *         '401': { description: 'assinatura inválida / parceiro não habilitado / sem capability de webhook' }
 *
