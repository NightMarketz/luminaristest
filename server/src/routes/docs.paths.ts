 ...
 *     post:
 *       summary: Execute ad-hoc KPI definitions against one dynamic table
 *       description: >-
 *         Stateless. Validates each KPI with Zod and against the target table's real column
 *         list, then computes them over the table rows and returns the results. Nothing is
 *         persisted. Each KPI is computed in its own try/catch, so one broken calculation
 *         yields a null value for that KPI instead of failing the whole response.
 *       tags: [Analytics]
 *       security: [{ bearerAuth: [] }]
 *       requestBody:
 *         required: true
 ...
 *                   type: string
 *                   format: cuid
 *                   description: The table every KPI in this request is computed over
 *                 kpis:
 *                   type: array
 *                   minItems: 1
 *                   items:
 ...
 *                         type: string
 *                         description: >-
 *                           Required by the schema but NOT read by the server, which computes
 *                           every KPI over the top-level tableId. Send the same value. See
 *                           ADR-ANALYTICS-DEFS F-AD6.4 item 1.
 *                       measure: { type: string, enum: [sum, avg, count, min, max] }
 *                       field:
 *                         type: string
 *                         minLength: 1
 *                         description: Column the measure aggregates. Required even for count.
 *                       filters:
 *                         type: array
 *                         items:
 *                           type: object
 ...
 *                         error: { type: string, nullable: true, enum: [Calculation failed] }
 *         '400': { description: Zod validation failed, or a KPI references a column absent from the table schema }
 *         '401': { $ref: '#/components/responses/UnauthorizedError' }
 *         '404': { $ref: '#/components/responses/NotFoundError' }
 *
