...
describe('NF-e fixture provenance (F0-3 merge gate)', () => {
  ...
  it('nenhum fixture é sintético (troque pela NF-e real p/ destravar o merge)', () => {
    const synthetic = xmlFiles().filter((f) =>
      readFileSync(join(FIXTURE_DIR, f), 'utf8').includes(SENTINEL),
    );
