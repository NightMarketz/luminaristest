...
| ADR | Título | Status | Data | Classe |
|---|---|---|---|---|
| [ANALYTICS-DEFS](ADR-ANALYTICS-DEFS-write-unblock.md) | Escrita de definições de analytics — destravar × apagar as rotas × manter congelado (forks F-AD0..F-AD6) | **Accepted (parcial) — F-AD0 → (c) MANTER CONGELADO, ratificado 2026-08-01**; barreira implementada e mutation-testada (`DynamicTableService.systemTableWriteLock.test.ts`, 8 casos). **F-AD5 (a tela) segue ABERTO** (decisão de produto); **F-AD6 → (a) MANTER `custom-kpis`, ratificado 2026-08-02** (o dono confirmou superfície de execução ad-hoc de KPI no roadmap; hoje essa categoria está vazia no repo — E22). `_REUSE-CRITERION.md` respondido: **(c) convergir ELIMINADA** (é clonar o morto) — quando a superfície ad-hoc existir, ela executa `PipelineSpec`, não `KpiDefinition`. **Ativa os consertos pendentes de F-AD6.4** (E18 `tableId` exigido-e-ignorado; E19 rota fora do `openapi.json`), não implementados. F-AD1..F-AD4 dormentes. Origem: achado N1 da revisão independente do PR #157 (`6500249`) | 2026-08-01 (emenda 2026-08-02) | DECISÃO ARQUITETURAL (DynamicTable / policy) |
