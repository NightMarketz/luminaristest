...
| **4. Furo do gate de wiring** | E17 | ✅ **FECHADO 2026-08-02** — `server/src/__tests__/route-spec-wiring.test.ts`. Reconstrói a **tabela de rotas do Express** (`_router.stack`, com o prefixo de montagem desmontado do regexp) e a cruza com a spec construída da fonte. **4 guards:** (1) sanidade do walker — piso de rotas + zero regexp não-parseado, que é o que impede o gate de **passar vacuamente**; (2) toda rota registrada tem entrada na spec **para aquele método**; (3) nenhum path da spec sem rota (doc morta) — hoje **0**, sem exceção; (4) a allowlist **não apodrece** (entrada que virou documentada falha). Medição na adoção: **146 paths `/api` registrados × 138 na spec → 9 lacunas**, congeladas em `KNOWN_UNDOCUMENTED` como **baseline que só encolhe**. **Mutation-testado (4/4):** rota nova sem doc → guard 2 vermelho; `decodeMount` quebrado → guard 1 vermelho (**e o guard 2 ficou verde sem ver nada — é a prova de por que o guard 1 existe**); entrada obsoleta na allowlist → guard 4 vermelho; rota documentada removida → guard 3 vermelho |
