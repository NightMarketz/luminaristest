...
- **Evidência:** `.claude/skills/_OPERATING-GATES.md` → regra **OPS-005** (nova + linha no mapa de enforcement); 7 PRs abertos ao fim da sessão, todos com review PASS e **nenhum mergeado**; execução literal da skill `luminaris-orchestrator` sobre a pergunta de sequenciamento → sem rota (ORCH-005); memorando da persona-orquestrador ("o código escrito nesta sessão não reduziu o gargalo — aumentou-o").
- **Como aplicar:** **OPS-005** — antes de abrir QUALQUER frente nova: `gh pr list --state open` (probe objetivo, não auto-reporte) + nomeie o gate bloqueante de cada item; **≥3 não-landados com gate aberto ⇒ relate a fila em vez de rotear**; **nunca empilhe sobre base com gate bloqueante aberto** (empilhar aposta N PRs num gate); exceção sempre permitida = trabalho que **drena** (rodar gate, resolver conflito, consertar PR, mergear). Regra geral, além do contábil: **diagnóstico que você quer que segure comportamento vira gate, não memória** — e o gate vive na camada sempre-ativa (OPS), não numa skill (skill só dispara se invocada; o modo de falha real foi ninguém invocar nada).
- **Durável?** sim → reforça [[accounting-gargalo-is-human-validation]] e [[ops-operating-gates]] (OPS-005).
