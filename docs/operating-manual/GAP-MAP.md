...
| **`deleteTableData` ignora `immutableAfter`/`lifecycle`** — regra declarada no schema ("registro `Paid` é imutável", `scope:'all'`) × caminho real: os Guards 2/3 rodam só em `updateTableData`; o delete roda `beforeDelete` → `deleteConstraints` → soft delete sem consultá-los. Um registro que não pode ser editado pode ser apagado. Contrato `[AC-2.2-4]` já nomeia; sem teste-guarda nem item de fila até 2026-09-21 | teste-guarda `server/src/features/dynamicTables/services/__tests__/DynamicTableService.integration.test.ts` › `Governance: immutableAfter › GAP-MAP 8: blocks DELETING a record that immutableAfter scope:all makes immutable (status = Closed)` — instrumentado VERMELHO 2026-09-25 (`it.failing`; "Received promise resolved instead of rejected" na asserção do delete, com o controle do update na mesma linha passando). Asserção neutra quanto ao fork (só exige que o delete lance e a linha continue viva) | **FECHADO 2026-09-26** — fork **(a)** do dono ("fork (a), corrige o GAP-MAP 8"): `deleteTableData` lança `ValidationError` quando uma regra `immutableAfter` **scope:'all'** está satisfeita na linha E em cada filha que o CASCADE apagaria (decisões do dono 26/09: só `scope:'all'` — regras de lista de campos seguem permitindo o delete; CASCADE sobre filha imutável bloqueia o pai). 2º caso: `Delete constraints › GAP-MAP 8: CASCADE is blocked when a referencing child row is immutable (scope:all)`, vermelho→verde no mesmo PR. `it.failing` → `it`. `lifecycle` no delete segue sem caso próprio | `awk '/async deleteTableData\(/,/^  }$/' server/src/features/dynamicTables/services/DynamicTableService.ts \| grep -c immutableAfter` → 0 |
...
8. **`deleteTableData` × `immutableAfter`/`lifecycle`** (Nível 3, 2026-09-21) — ✅ **instrumentado vermelho 2026-09-25** (autorização do dono em chat: "instrumenta o GAP-MAP 8"): `DynamicTableService.integration.test.ts` › "GAP-MAP 8 …" (`it.failing`). Prova: `cd server && npx jest --selectProjects integration --runInBand --testPathPatterns DynamicTableService.integration -t "GAP-MAP 8"`. **FECHADO 2026-09-26** (mesmo PR): fork (a) — guard `immutableAfter scope:'all'` no delete e no CASCADE; `it.failing` → `it`. Prova: mesmo comando, sem `-t` (37/37).
