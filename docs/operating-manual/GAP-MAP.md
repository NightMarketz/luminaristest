...
| **CRM — analytics ignora oportunidades** — `CrmAnalyticsService.getAnalytics` só lê `leads`; opp Ganha (portadora de receita/título) não entra em Ganhos/ticket. Decisão do dono 2026-09-25 (AskUserQuestion): "Incluir oportunidades" | teste-guarda `server/src/features/crm/services/__tests__/CrmAnalyticsService.test.ts` › `conta a oportunidade Ganha em wonLeads e no ticket médio` | [INSTRUMENTADO 2026-09-25] — `it.failing` enquanto a lacuna está aberta (padrão `UniqueFieldConcurrency`): o fix faz o teste "falhar" e ele vira `it` | `cd server && npx jest CrmAnalyticsService.test` |
