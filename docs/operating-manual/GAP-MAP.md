...
| **`deleteTableData` ignora `immutableAfter`/`lifecycle`** — regra declarada no schema ("registro `Paid` é imutável", `scope:'all'`) × caminho real: os Guards 2/3 rodam só em `updateTableData`; o delete roda `beforeDelete` → `deleteConstraints` → soft delete sem consultá-los. Um registro que não pode ser editado pode ser apagado. Contrato `[AC-2.2-4]` já nomeia; sem teste-guarda nem item de fila até 2026-09-21 | teste-guarda `server/src/features/dynamicTables/services/__tests__/DynamicTableService.integration.test.ts` › `Governance: immutableAfter › GAP-MAP 8: blocks DELETING a record that immutableAfter scope:all makes immutable (status = Closed)` — instrumentado VERMELHO 2026-09-25 (`it.failing`; "Received promise resolved instead of rejected" na asserção do delete, com o controle do update na mesma linha passando). Asserção neutra quanto ao fork (só exige que o delete lance e a linha continue viva) | [INSTRUMENTADO 2026-09-25] — próximo passo = decisão do dono sobre o fix (guard no delete × `deleteConstraints` RESTRICT no pai), depois `sessao-correcao` (`it.failing` → `it`). `lifecycle` no delete não ganhou caso próprio (um teste por lacuna) | `awk '/async deleteTableData\(/,/^  }$/' server/src/features/dynamicTables/services/DynamicTableService.ts \| grep -c immutableAfter` → 0 |
