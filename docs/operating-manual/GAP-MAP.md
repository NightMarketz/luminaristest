...
7. **`unique`/`compositeUnique` sem gate in-tx** (Nível 4, 2026-09-21) — ✅ instrumentado 2026-09-23 (`UniqueFieldConcurrency.integration.test.ts`, `it.failing`); falta a correção; conserto reusa `withTableWriteLock` + re-check in-tx. Não é aparato novo: é a mesma Fase 3 aplicada à regra vizinha. Beneficia CRM/vendas, não só contabilidade.
