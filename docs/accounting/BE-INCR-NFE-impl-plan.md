...
| **F0-3** | 🟡 **PLACEHDER SINTÉTICO em disco + trava de merge armada** (2026-07-23) | `*.SYNTHETIC.xml` compra multi-item + venda, ancorados na F0-2 | `server/src/lib/__tests__/fixtures/nfe/*.SYNTHETIC.xml` + `nfe-fixture-provenance.test.ts` + `fixtures/nfe/README.md` | `sintetico-nao-cobre-formato-de-dado-real`: o sintético prova a mecânica, não o leiaute. Fase A **DESTRAVADA para construir**; o **merge fica travado** pelo teste de proveniência (falha enquanto houver marcador `SYNTHETIC-FIXTURE-NOT-REAL`; check `Server – typecheck & test` é obrigatório). **Troca pela NF-e 4.00 real anonimizada do dono → verde → destrava.** |
