...
| **RN** | **BE-INCR-EVENT-VOCAB-RENAME** — renomear o vocabulário de eventos `salon.*` → `sale.*` neutro nas pontes de `features/accounting/sync/**` | BE increment (lacuna do P1; **ADR/BRIEF próprios pendentes**) | **NOVO 2026-08-25 — ENTRA NA FRENTE DO P2.** Consequência direta de **F-P2-6 → (b) RATIFICADO**: as pontes emitem `salon.sale.finalized` etc. literalmente, então o binding de QUALQUER vertical não-salão vincularia um vocabulário errado por construção. O dono ratificou que o rename é **ciclo próprio** (instrumentação → correção), **não** emenda ao perímetro no meio do P2 — `sync/**` está DENTRO do perímetro zero-diff, e mexer nele durante o P2 tornaria negociável o critério que o P2 existe para julgar. **Ainda exige ADR + BRIEF + sinal humano próprios (ORCH-006)** — a ratificação do F-P2-6 decidiu QUE ele acontece e em que ordem, não autorizou a execução. |
