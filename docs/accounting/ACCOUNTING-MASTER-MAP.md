...
| **Baixa parcial em AP/AR** | ⚫→⏳ **autorizado a abrir ADR 2026-09-03 (F-M3)** | Hoje **rejeitada com 400 explícito** (`ReceivableService.ts:213`, `PayableService.ts:316`). N recibos por título, aging por saldo, CAS por saldo. Gatilho da tese do produto já registrado. |
| **Fluxo de caixa projetado** (read-only) | ⚫→⏳ **autorizado BRIEF 2026-09-03 (F-M3)** | Vencimentos AP/AR por janela sobre dados existentes; sem migração. 0 ocorrências de projeção hoje. |
...
| **Envio de ECD/ECF ao contador por e-mail** | ⚫→⏳ **autorizado a abrir ADR 2026-09-03 (F-M4)** | Era A6 do `ROADMAP-PLATAFORMA.md`. Outbound ⇒ confirmação explícita por envio; dado contábil saindo do processo ⇒ decisão de LGPD no mesmo ADR. |
| **Telas do já-existente sem consumidor FE** | ⏳ **autorizado BRIEF 2026-09-03 (F-M2/F-M4)** | `audit/verify-chain` (0 consumidor), `source-documents` por lançamento (0), `sped/ecf/real/generate` (0), `referential/catalog/import` (0). Dois BRIEFs: `FE-INCR-AUDIT-PROVENANCE` (contábil) e `FE-INCR-COMPLIANCE-2` (fiscal). |
...
|---|---|---|---|---|
| **Contábil** | ~~13/17~~ **13/19 (68%)** | 13/22 | tela `verify-chain`, tela `source-documents`, e-mail ECD/ECF ao contador (ADR, agora com gancho legal do PNCT), pendências do reconcile (F-W2F-3/5 → BRIEF), **imobilizado/depreciação (ADR, F-Z0)**, **retificação ECD/ECF** | B-4, H1 (ECD), H2 |
| **Financeiro** | 13/17 (76%) | 13/18 | NF-e compra (em voo), baixa parcial AP/AR (ADR), caixa projetado (BRIEF), remessa CNAB/boleto/Pix (ADR + dado do banco) | H2 (upload por clique) |
| **Fiscal** | ~~4/12~~ **4/13 (31%)** | 4/15 | tela import catálogo, botão ECF Real, blocos L/M/N, NF-e, apuração de tributos (ADR), EFD-Contribuições (ADR, raso, por último), DCTFWeb (ADR), D3 sob Lucro Real, **emissão de DF-e via parceiro emissor (ADR, F-M7)** | X2, H1, H1 2ª passada, **parceiro emissor + certificado** |
