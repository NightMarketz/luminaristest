...
>
> **⏩ ESTADO VIGENTE = FOLD 2026-09-17 (abaixo): régua 44/57 (C6b ✅ #340); orquestração vigente = `PROXIMOS-PASSOS-2026-09-17.md`; os "em voo"/"41/57"/"42/57"/"43/57" dos banners anteriores são HISTÓRICOS.**
>
...
>
> **🔁 FOLD 2026-09-17 — C6b MERGEADO (3 PRs seriais, "executa C6b" 16/09).** #337 PR-1 `daf76279` (período nos exports) · #338 PR-2 `15c8bf53` (`EXPORT_BANK_RECONCILIATION` + `EXPORT_ENTRY_SAMPLE`) · **#340 PR-3 `373d00d4`** (`AccountingDeliveryItem` tabela filha + migração aditiva com backfill, manifesto N-ário, `extraJobIds` com `resolveExtras` dentro da tx do confirm, `GET/PUT /delivery/profile`, openapi 188→189). Review independente do PR-3: F1 (migração retry-safe: `ADD COLUMN packageProfile` por último) → fix `63f02b04` → **PASS não gravado no PR** (veredito ficou no chat da sessão de review — lacuna de rastreabilidade declarada; precedente #338 grava no corpo). CI 5/5 no `63f02b04`. Docs: #339 `d7181f8f` (transcrição C12 J930/0930 + F-C12-5..7 + `PROXIMOS-PASSOS-2026-09-17`). **Régua: 44/57 — contábil 19/22 · financeiro 17/19 · fiscal 8/16.** Fila §5.1 sem nó em voo: C12 · C8 · FE-LALUR PR 2 `ready` sem "executa"; SEED-MY em B-4. Residual do C6b = `FE-INCR-DELIVERY` (consome `files[].kind` novo — plano §5.1).
>
...
> [GRAFO-DEPENDENCIAS-2026-09-11.md](GRAFO-DEPENDENCIAS-2026-09-11.md) (6 arestas mortas, 8 nós novos, cadeia crítica =
...
> período, sem binding, sem pipeline CRM. 56 achados → [plano em grafo](ONBOARDING-WIZARD-plano-grafo-brief.md)
...
> humano:** sign-off de browser da aba ([RUNBOOK-H2-WIZARD-ENTREVISTA](RUNBOOK-H2-WIZARD-ENTREVISTA.md), em branco).
>
...
## 3. Fila de integração travada em dado externo — BE-INCR-NFE **implementado fora de `main`** — ✅ **MERGEADO 2026-09-03 (#267 `9fbe200f`; ver fold no topo)**

> **🔁 EMENDA 2026-09-03 — a NF-e DEIXOU de ser "fila travada em dado externo".** Oito decisões do
> dono ([CEDULA-DECISAO-2026-09-03-integracao.md](CEDULA-DECISAO-2026-09-03-integracao.md) §B):
...
> | **C11** revisão profissional editável | B — nó novo do re-baseline 10/09 | ✅ **MERGEADO 2026-09-16 — PR #334 `a2c974cb`** (BRIEF #321; forks 16/09 todos a; review FAIL B1 → fix → delta PASS; CI 10/10). **Contábil 17/22 → 18/22.** Residual = `FE-INCR-REVIEW` (aba na Compliance, incremento irmão, sem item de fila) | `BE-INCR-REVIEW-LAYER-brief.md` |
...
> | **C6b** pacote ampliado ao contador | B — nó novo do re-baseline | ✅ BRIEF em `main` (#324 `ce0c97e8`); 5 forks ✅ ratificados 16/09 (todos a); C11 ✅ #334 → `ready`, falta "executa". **Plano granular 16/09** (3 PRs seriais: período nos exports → conciliação+amostra → tabela filha/extras) F-C6b-6..8 ✅ (a) ratificados 16/09; **"executa" dado 16/09** — ✅ **MERGEADO 2026-09-17**: PR-1 #337 `daf76279` · PR-2 #338 `15c8bf53` · PR-3 #340 `373d00d4` (review F1 → fix `63f02b04`; PASS não gravado no PR). **Contábil 18/22 → 19/22.** Residual = `FE-INCR-DELIVERY` (fora da régua) | `BE-INCR-CONTADOR-PACKAGE-EXTENDED-brief.md` · `…-execution-plan.md` |
...
| **LAC-B** | **UI da Prensa de binding** — ativação self-service (endpoint "ativar binding padrão do setor" embutindo a fixture, lugar natural = onboarding; editor da matriz papel→conta só quando um tenant divergir do padrão) | FE/BE ⏳ **ATIVADA 2026-09-07 pelo dono** ("Ativar agora", via `AskUserQuestion`; gatilho "onboarding self-service" do BRIEF dado pelo [plano em grafo](ONBOARDING-WIZARD-plano-grafo-brief.md) nó I3) | ~~O CLI atende o dono até onboarding self-service ou P2 exigirem~~ → **executar pelo BRIEF [FE-INCR-BINDING-ACTIVATION](FE-INCR-BINDING-ACTIVATION-brief.md) + emenda F-I3-1 (a)**: o DTO ganha `openCurrentPeriodIfMissing` (o compile exige período OPEN do mês corrente, degrau que o BRIEF não cobria). Depende de I1 (unidade) para I4 (chamada automática no onboarding). Registrado aqui para a lacuna não ficar sem dono: hoje **zero** referência a `accounting-binding` no `my-app`, e o boot aborta sem binding `Active` — o primeiro cliente que se instalar sozinho transforma isto em bloqueio. |
| **ONB** | **Plano em grafo do wizard de onboarding** — 19 nós (N0 Bearer ✅ #271; I1 primeira unidade; I1b backfill; I2 T0; I3 LAC-B; I4 onboarding ativa binding; I5 venda sem mapper = blocked; I6 unitId validado; I7 reset; I8 CRM; I9 analytics; I10 runbook M2; I11 agente; W1–W7 superfície do wizard) | plano ⏳ (forks ratificados: LAC-B, F-I3-1, F-I1-3, F-I8-1; 20 pendentes) | [ONBOARDING-WIZARD-plano-grafo-brief.md](ONBOARDING-WIZARD-plano-grafo-brief.md). Ordem é sinal do dono; espinha = N0→I1→I3→I4 (+I5). I1 e I2 colidem na tx do `installPresetAsSystem`; I1 e I8 colidem no body do `POST /dashboard/create` → serial. |
...
|---|---|---|---|
| **Contábil** | ~~16/19 (84%)~~ ~~16/22 (73%)~~ ~~17/22 (77%, fold 2026-09-11: C6 CONTADOR-DELIVERY ✅ #305 `7725f0ca`)~~ ~~18/22 (82%, fold 2026-09-16: C11 REVIEW-LAYER ✅ #334 `a2c974cb`)~~ **19/22 (86%, fold 2026-09-17: C6b PACKAGE-EXTENDED ✅ #337/#338/#340 `373d00d4`)** | **+3**: ~~revisão profissional editável~~ ✅ C11 #334 (resposta 2 + F-EDIT-1 → a+c: edita o DADO ou lança acerto, e regera — nunca o arquivo); ~~pacote ampliado ao contador~~ ✅ C6b #340 (resposta 8 reabre F-CD3: balancete, razão, conciliação, amostra); endurecimento dos campos de identidade da geração SPED (resposta 3 — qualificação J930 vira enum do manual, CPF/CNPJ/UF com máscara) | retificação passa a **preservar a versão anterior** (resposta 7); tabela de depreciação vira **editável por tenant** semeada do Anexo III (resposta 6) |
