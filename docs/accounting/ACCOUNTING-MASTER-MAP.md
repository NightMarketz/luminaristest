...
ao rebase:** a fase-b nasceu **antes** do rename RN (PR #222) e carrega **11 ocorrências** de
...
acha. O rebase (hoje 23 commits, `git rev-list --count origin/claude/nfe-fase-b..origin/main`) DEVE
aplicar o vocabulário `sale.*` antes de qualquer merge. `claude/nfe-fase-a` fica como histórico.
