...
| **P2** | **BE-INCR-P2-VERTICAL-CLINICA** — o 2º vertical (clínica estética) que prova a prensa de binding: preset próprio + ficha clínica + binding compilado + ECD gerada pelo mesmo motor, com `git diff` vazio no perímetro zero-diff | BE increment (BRIEF pronto, **forks PENDENTES**, código NÃO iniciado) | **NOVO 2026-08-25 — passa a ser o item de CÓDIGO da fila** (o 0 mergeou; o resto do Bloco A segue gate humano/dado externo). [BRIEF](BE-INCR-P2-VERTICAL-CLINICA-brief.md) (PR #214) — 11 comportamentos, contratos esboçados, **8 forks PENDENTES: NENHUM ratificado**. Herdados do [ADR-P2](../adr/ADR-P2-second-vertical.md): **F-P2-3** (profundidade da prova — aberto **por dependência do H1/PVA**, não por indecisão) e **F-P2-4** (onde registrar *time-to-first-ECD*). Novos do BRIEF: **F-P2-5** mecanismo da ficha clínica · **F-P2-6** vocabulário `salon.*` + onde mora o gate de cobertura · **F-P2-7** parametrização do CLI de ativação · **F-P2-8** amplitude operacional da prova · **F-P2-9** `SOFT_CLOSED`×`HARD_CLOSED` na fixture · **F-P2-10** perímetro zero-diff de `dynamicTablesController.ts` e `presets/ai/`. ⛔ **F-P2-10 é BLOQUEANTE de execução, não cosmético:** o comportamento 3 (o matcher tem de resolver "clínica" para o preset novo, e não para `beautySalon`, cuja descrição hoje **já reivindica** "clínicas de estética" em `PresetKnowledgeBase.ts:17`) exige editar um arquivo que a classificação vigente põe em zero-diff estrito — ou o perímetro se ajusta, ou a prova reprova por fazer exatamente o que o ADR-P2 §1 autoriza. Junto com F-P2-5, é o que decide **em qual arquivo a primeira linha é escrita**. ⚠️ **Footgun verificado em disco (insumo do F-P2-7):** [activateAccountingBindingCli.ts:61,123](../../server/src/jobs/activateAccountingBindingCli.ts:61) deriva `sectorKey` da flag `--sector-key` mas grava **sempre** `SALON_BINDING_V1.eventBindings` — rodar `--sector-key aestheticClinic` hoje ativaria o binding **do salão** sob rótulo de clínica: linha `Active` válida, `descriptionTemplate` errado, zero erro. **Pré-condição §5.2 do roadmap ("vertical 1 validado, PVA verde + sign-offs") INSATISFEITA** — itens 3 e 4 deste bloco. ADR-P2 ainda **`Draft`**. **Nada roteia daqui sem ratificação do dono (ORCH-006).** |
