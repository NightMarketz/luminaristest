...
| **Baixa parcial em AP/AR** | **ADR Proposed ✅ #276 `b7a62a73` (2026-09-07) + parecer do arquiteto — 7 forks F-PS1..F-PS7 RATIFICAÇÃO PENDENTE** | Hoje **rejeitada com 400 explícito** (`ReceivableService.ts:213`, `PayableService.ts:316`). N recibos por título, aging por saldo, CAS por saldo. **Achado do ADR:** o CAS aritmético original não era expressável em Prisma como escrito — reformulado; **2 tie-outs a corrigir** antes da implementação. `ADR-INCR-PARTIAL-SETTLEMENT.md` + `PARECER-ARCHITECT-ADR-INCR-PARTIAL-SETTLEMENT.md`. |
| **Fluxo de caixa projetado** (read-only) | **BRIEF ✅ #278 `77c77553` (2026-09-07) — 7 forks F-CF1..F-CF7 RATIFICAÇÃO PENDENTE** | Vencimentos AP/AR por janela sobre dados existentes; sem migração. 0 ocorrências de projeção hoje. BRIEF exige um report BE read-only novo, sem migração. `FE-INCR-CASH-FORECAST-brief.md`. |
...
| **Envio de ECD/ECF ao contador por e-mail** | **ADR Proposed ✅ #274 `239945d1` (2026-09-07) + parecer do arquiteto — 8 forks F-CD1..F-CD8 RATIFICAÇÃO PENDENTE** | Era A6 do `ROADMAP-PLATAFORMA.md`. Outbound ⇒ confirmação explícita por envio; dado contábil saindo do processo ⇒ decisão de LGPD no mesmo ADR. **CONDICIONADO ao item 0 do pedido ao contador (F-Z0)** — se a resposta for "não", este item reabre (regra já registrada linha acima, "camada zero"). `ADR-CONTADOR-DELIVERY.md` + `PARECER-ARCHITECT-ADR-CONTADOR-DELIVERY.md`. |
| **Telas do já-existente sem consumidor FE** | **BRIEF ✅ ambos, 2026-09-07 — `FE-INCR-AUDIT-PROVENANCE` #277 `aafe8a5e` (7 forks F-FEAP-1..7 pendentes) + `FE-INCR-COMPLIANCE-2` #279 `5daa8763` (7 forks F-COMP2-1..7 pendentes, regra do lote `accounting.json` 847/847 — medir a cada aplicação na Fase B)** | `audit/verify-chain` (0 consumidor), `source-documents` por lançamento (0), `sped/ecf/real/generate` (0), `referential/catalog/import` (0). Dois BRIEFs: `FE-INCR-AUDIT-PROVENANCE` (contábil) e `FE-INCR-COMPLIANCE-2` (fiscal). |
...
|---|---|---|---|---|
| **Contábil** | ~~13/17~~ **13/19 (68%)** | 13/22 | tela `verify-chain` + tela `source-documents` (BRIEF ✅ #277 `aafe8a5e`, 2026-09-07, 7 forks pendentes), e-mail ECD/ECF ao contador (ADR Proposed ✅ #274 `239945d1` + parecer, 2026-09-07, 8 forks pendentes, condicionado ao F-Z0), pendências do reconcile (BRIEF ✅ #275 `1bbeb953`, 2026-09-07 — Fork 1 fechado pela cédula, 5 forks pendentes), **imobilizado/depreciação (ADR, F-Z0)**, **retificação ECD/ECF** | B-4, H1 (ECD), H2 |
| **Financeiro** | 13/17 (76%) | 13/18 | NF-e compra (em voo), baixa parcial AP/AR (ADR Proposed ✅ #276 `b7a62a73` + parecer, 2026-09-07, 7 forks pendentes, CAS reformulado + 2 tie-outs a corrigir), caixa projetado (BRIEF ✅ #278 `77c77553`, 2026-09-07, 7 forks pendentes), remessa CNAB/boleto/Pix (ADR + dado do banco) | H2 (upload por clique) |
| **Fiscal** | ~~4/12~~ **4/13 (31%)** | 4/15 | tela import catálogo + botão ECF Real (BRIEF ✅ #279 `5daa8763`, 2026-09-07, 7 forks pendentes), blocos L/M/N, NF-e, apuração de tributos (ADR), EFD-Contribuições (ADR, raso, por último), DCTFWeb (ADR), D3 sob Lucro Real, **emissão de DF-e via parceiro emissor (ADR, F-M7)** | X2, H1, H1 2ª passada, **parceiro emissor + certificado** |
