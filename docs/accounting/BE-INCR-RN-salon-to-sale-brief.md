...
| A ponte **engole** esse erro e loga `error` (não `warn` — mapper ausente não está em `SYNC_SKIP_ERROR_CODES`, [`AccountingSyncPort.ts:91`](../../server/src/features/accounting/sync/AccountingSyncPort.ts:91); só período-fechado/MAX_CENTS caem no ramo `warn`) — venda grava, lançamento não nasce, HTTP 200, "left for reconciliation" | [`SalonSalesAccountingBridge.ts:108-125`](../../server/src/features/accounting/sync/bridges/SalonSalesAccountingBridge.ts:108) (achado do BRIEF do P2, reconfirmado aqui) |
...
| `JournalEntry.sourceType`/`StockMovement.sourceType` são `String` livre (não enum) — sem migração de schema Prisma necessária; o risco é **de dado**, não de shape | [`schema.prisma:493,742,1146`](../../server/prisma/schema.prisma:493) |
...
### 4.3 `JournalEntry.sourceType` / `StockMovement.sourceType` (Prisma — sem migração de schema)
